...
const updateAccessToken = async (req, res, next) => {
    try {
        ...
        const decoded = jsonwebtoken_1.default.verify(refresh, process.env.REFRESH_TOKEN);
        ...
        await redis_1.redis.set(user._id, JSON.stringify(user), "EX", 604800); //7 day expiration
        next();
    }
    catch (error) {
        return next(new ErrorHandler_1.default(error.message, 400));
